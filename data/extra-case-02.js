  ...
  patron: {
    ...
    reactions: {
      ...
      low: {
        ...
        line:
"ありがとうございます。少し整理できてきました。最初はAPCのことが気になっていたんですが、そもそも何のためにOAを考え始めたのかも含めて整理できるとよさそうですね。",
      },
    },
  },
  facts: {
  ...
  questions: [
    ...
    },
    {
      ...
      text: "今回、その雑誌を投稿先に選ばれたのは、どのような理由からですか？",
      requiresAll: [],
      requiresAny: [],
      response:
        "研究分野との相性や、この分野の研究者によく読まれていることを重視して選びました。OAについては、投稿したあとに意識したくらいです。",
      responseVariants: [
        "研究内容に合う雑誌だったことが一番の理由です。OAかどうかは、投稿先を選ぶときにはあまり意識していませんでした。",
      ],
    ...
    {
      id: "q_ex2_purpose",
      text: "今回、OAについて特に確認しておきたいのは、どのようなことでしょうか？",
      requiresAll: ["institution_notice"],
      requiresAny: [],
      response:
        "まずは、大学から案内された即時OAにきちんと対応しておきたいですね。もちろん広く読んでもらえるのもうれしいですが、今回は制度への対応がきっかけです。",
      responseVariants: [
        "必要なOA対応を間違えずに済ませたい、というのが一番近いですね。",
      ],
    ...
    },
    {
      ...
      text: "出版社のOAオプション以外の公開方法については、何か検討されていますか？",
      requiresAll: ["compliance_goal"],
      requiresAny: ["publisher_oa_offer", "apc_concern"],
      response:
        "大学の機関リポジトリは候補になるのかなと思っています。ただ、今回の論文でも使えるのか、即時OAへの対応になるのかまでは分かっていなくて。",
      responseVariants: [
        "機関リポジトリという方法があることは知っています。ただ、今回それが利用できるのかまでは確認できていません。",
      ],
    ...
    {
      id: "q_ex2_policy",
text: "著者最終稿の公開条件については、出版社のポリシーをもう確認されていますか？",
      requiresAll: ["accepted_manuscript", "repository_awareness"],
      requiresAny: [],
response:
  "そこまではまだ確認していません。雑誌によって、公開できる版や時期が違うんですよね。そこも確認できると安心ですね。",

responseVariants: [
  "まだ確認できていません。リポジトリがあっても、どの論文でもすぐ公開できるわけではないんですよね。公開条件も確認しておきたいです。",
],
