...
test("雑談を6問目より前に深掘りすると質問回数が1〜3回からランダムに回復する", () => {
  ...
  [
    ...
    { random: 0.999999, expected: 3 },
  ].forEach(({ random, expected }) => {
    const session = new GameSession(caseData, { random: () => random });
    session.askQuestion(cheerful.id);
    const result = session.askQuestion(followUp.id);
    assert.equal(result.recoveredQuestions, expected);
    assert.equal(session.state.questionBonus, expected);
    assert.equal(session.getQuestionLimit(), 6 + expected);
  });
});

test("雑談を深掘りして2回回復した場合は合計8問まで続けられる", () => {
  CASES.forEach((caseData) => {
    ...
    const recoveredSession = new GameSession(caseData, { random: () => 0.5 });
...
test("CASE 02: 資料条件の4択が会話の条件と明確に対応する", () => {
  const case02 = CASES.find((caseData) => caseData.id === "case02");
  const sourceSlot = case02.deduction.slots.find((slot) => slot.id === "source");
  const optionText = Object.fromEntries(
    sourceSlot.options.map((option) => [option.id, option.text]),
  );

  assert.deepEqual(optionText, {
    source_flexible: "前に見た本と同じものでなくてもよい",
    source_book: "ウェブ資料ではなく本を希望している",
    source_practical: "図解と実例のある実用資料を希望している",
    source_any: "以前見た資料を手がかりにしている",
  });

  getCaseCombinations(case02).forEach((combination, index, combinations) => {
    const session = new GameSession(case02, {
      random: () => (index + 0.1) / combinations.length,
    });
    assert.match(
      session.caseData.correctSentence,
      combination.source.optionId === "source_flexible"
        ? /前に見た本と同じものでなくてもよい/
        : combination.source.optionId === "source_book"
          ? /ウェブ資料ではなく本を希望している/
          : /図解と実例のある実用資料を希望している/,
    );
  });
