...
test("回答確定時は青を基本とし、正答数に応じて金・虹を抽選する", () => {
  ...
  assert.equal(selectCutin(score(4), () => 0.59), "gold");
  assert.equal(selectCutin(score(4), () => 0.6), "blue");

  assert.match(files["js/audio.js"], /decision\.mp3/);
  assert.match(files["js/audio.js"], /decision_normal\.mp3/);
  assert.match(files["js/audio.js"], /\["blue", "gold"\]\.includes\(cutin\)/);
  assert.match(files["js/audio.js"], /choice\.mp3/);
  assert.match(files["js/app.js"], /action === "choose-slot-option"[\s\S]*?playChoiceSound\(\)/);
  assert.match(files["js/app.js"], /getDebugCutin\(\) \?\? selectCutin\(score\)/);
  assert.match(files["js/app.js"], /playDecisionSound\(cutin\)[\s\S]*?showCutin\(cutin, \{/);
  assert.match(files["js/cutin.js"], /data-cutin-overlay/);
  assert.match(files["js/cutin.js"], /cutin_blue_det\.webp/);
  assert.match(files["js/cutin.js"], /cutin_gold_det\.webp/);
  assert.match(files["js/cutin.js"], /cutin_rainbow_det\.webp/);
  assert.match(files["data/extra-case.js"], /cutinAssetVariant: "det"/);
  assert.match(files["js/app.js"], /assetVariant: currentCase\.presentation\?\.cutinAssetVariant/);
...
test("キーボード・ラベル・色以外の正誤・モーション抑制を備える", () => {
  ...
  assert.match(files["js/ui.js"], /<section class="slot-card">/);
  assert.match(files["js/ui.js"], /data-action="open-slot-choice"/);
  assert.match(files["js/ui.js"], /id="answer-choice-dialog"/);
  assert.match(files["js/ui.js"], /data-answer-choice-options/);
  assert.doesNotMatch(files["js/ui.js"], /<select data-action="select-slot"/);
  assert.match(files["css/style.css"], /\.answer-choice-card\s*\{[\s\S]*?justify-content:\s*center[\s\S]*?text-align:\s*center/);
  assert.doesNotMatch(files["js/app.js"], /answer-choice-card__number/);
  assert.doesNotMatch(
    files["js/ui.js"],
...
test("回答選択時は画面を再描画せず、スクロール位置を維持する", () => {
  assert.match(
    files["js/app.js"],
    /function updateDeductionSelection\(\)[\s\S]*?sentence\.textContent = session\.getDeductionSentence\(\)/,
  );
  assert.match(
    files["js/app.js"],
    /action === "choose-slot-option"[\s\S]*?updateDeductionSelection\(\)/,
  );
  assert.doesNotMatch(
    files["js/app.js"],
    /action === "choose-slot-option"[\s\S]*?renderDeduction\(\)[\s\S]*?return/,
  );
});
...
test("質問回数の回復案内は利用者の返答が表示し終わってから出す", () => {
  ...
  assert.match(
    files["js/app.js"],
    /if \(targetIndex >= targets\.length\)[\s\S]*?revealConversationNotices\(\)[\s\S]*?button\.disabled = false/,
  );
  assert.match(
    files["js/app.js"],
...
test("時間案内は最後の利用者の言葉を表示し終えてから出す", () => {
  assert.match(
    files["js/ui.js"],
    /class="limit-notice" role="status" data-limit-notice hidden/,
  );
  assert.match(
    files["js/app.js"],
    /const revealLimitNotice = \(\) => \{[\s\S]*?limitNotice\.hidden = false/,
  );
  assert.match(
    files["js/app.js"],
    /if \(targetIndex >= targets\.length\)[\s\S]*?revealConversationNotices\(\)/,
  );
  assert.match(
    files["css/style.css"],
    /\.limit-notice\[hidden\]\s*\{\s*display:\s*none/,
  );
});
